...
以下时序图展示了云产品组件通过 [[PCM/PCM/index|PCM]] 获取派生凭证的核心业务流程，包含多级缓存检查、凭证派生交互以及异常降级路径。
...
以下时序图展示了当发生 AK 被禁用导致访问报错时，运维排查人员进行日志分析、状态确认、应急处置及反馈研发的完整业务流程。
...
sequenceDiagram
    ...
    运维排查人员->>网关日志系统: 遇到访问报错，查询拦截日志
    网关日志系统-->>运维排查人员: 返回拦截日志记录
    运维排查人员->>运维排查人员: 怀疑PCM禁用AK导致，提取日志中的请求AK
    运维排查人员->>PCM服务: 查询提取的AK状态
    PCM服务-->>运维排查人员: 返回AK状态（已禁用）
    运维排查人员->>应急处置流程: 采用应急处置方案进行处置
    应急处置流程-->>运维排查人员: 处置完成
    运维排查人员->>研发团队: 反馈研发侧排查禁用原因
```