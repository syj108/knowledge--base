...
当 SDK 请求 Core 失败（网络不通/超时/Core异常）时，触发以下降级逻辑：
1. **第二层：读取本地加密文件**（AES-256-GCM解密）
   - 如果本地文件有有效凭证：返回本地缓存凭证 (code=301/302)
   - 如果本地文件也没有：降级返回底表AK (code=401)

### 派生 AK 队列轮转保护机制

派生 AK 队列会持续轮转（定期创建新 AK、禁用老 AK），但在以下两种情况下会暂停轮转，以保护正在使用中的凭证（排查轮转停止问题时可参考）：
