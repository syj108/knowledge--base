...
*   **核心组件**：
    *   **PCM Core**：部署在**两个 Docker 容器**上，日志排查时需同时查询这两个容器。
    *   **PCM Controller**：策略中心，提供日志查询关联能力。
...
| 组件 | 日志类型 | 日志路径/位置 | 日志内容 | 轮转策略与注意事项 |
| --- | --- | --- | --- | --- |
| PCM Controller | API/异常日志 | `/home/admin/pcm_controller/logs/api/logs/` | API 请求日志、异常请求、定时任务报错等 | 需确认日志轮转配置是否正常。若出现超大文件导致磁盘打满，需手动清理历史日志，并排查是否有大量异常请求或定时任务循环报错。 |
| PCM Core | Error 日志 | `/opt/tengine/logs/error.YYYY-MM-DD.log` | 记录 PCM Core 报错返回信息 | 按天轮转。 |
| PCM Core | Access 日志 | `/opt/tengine/logs/access.YYYY-MM-DD.log` | 记录 PCM Core 接收到的请求信息，包含 `limit_req_status` 字段 | 按天轮转。用于排查 HTTP 502 限流问题，可通过 `tsar -l -i 1 --nginx` 查看 QPS。 |
| PCM Core | AK 申请日志 | 通过 PCM Core 获取 | 记录每个 IAMID 申请派生 AK 记录（底表 secretARN 缓存时间为 12 小时，持续使用的产品理论上每 12 小时一条记录） | - |
| Go SDK | 运行日志 | 应用本地日志文件 | SDK 运行日志 | **2512 之前版本存在日志轮转 Bug**，会导致文件持续增长。需升级至 2512 及以上版本。临时处理：使用 `> logfile` 截断日志文件（切勿 `rm` 正在写入的文件）。 |
| 客户端/Java SDK | 标准输出/日志 | 应用标准输出/日志文件 | `Failed to refresh credential` 等 WARN 日志，或线程阻塞堆栈 | 当 PCM 服务端未部署或不可达时，SDK 降级会产生大量 WARN 日志，不影响业务但可能触发监控告警。 |
...
### 通用日志排查思路
> **注意**：在进行 PCM Core 日志排查时，**必须同时检查 PCM Core 部署的两个 Docker 容器**。

**1. 排查 Error 日志（确定是否 PCM Core 报错返回）**
*   **有具体 RequestID**：直接查询对应日期的 error 日志。
    ```bash
    grep -rn "您的RequestID" /opt/tengine/logs/error.YYYY-MM-DD.log
    ```
*   **无具体 RequestID**：根据 AKID、IAMID 和时间段进行复合筛选。
    ```bash
    grep "您的AKID" /opt/tengine/logs/error.YYYY-MM-DD.log | grep "您的IAMID" | awk '$1 >= "YYYY/MM/DD" && $2 >= "HH:MM:SS" && $2 <= "HH:MM:SS"'
    ```

**2. 排查 Access 日志（确定是否 PCM Core 接收到请求）**
*   **有具体 RequestID**：直接查询对应日期的 access 日志。
    ```bash
    grep -rn "您的RequestID" /opt/tengine/logs/access.YYYY-MM-DD.log
    ```
*   **无具体 RequestID**：根据 AKID 和时间段进行复合筛选。
    ```bash
    grep -E '"time_local": "(DD/Mon/YYYY:HH:MM|DD/Mon/YYYY:HH:MM)' /opt/tengine/logs/access.YYYY-MM-DD.log | grep "您的AKID"
    # 或精确到秒
    grep "您的AKID" /opt/tengine/logs/access.YYYY-MM-DD.log | grep -E '"time_local": "DD/Mon/YYYY:HH:MM:SS'
    ```
...
### 派生 AK 队列轮转停止排查
若发现派生 AK 队列停止轮转，请排查以下保护机制与状态是否被触发：
1.  **保护一（最新派生 AK 保护）**：检查即将禁用的 AK 是否为某产品获取的“最新”派生 AK。若是，需等待该产品获取更新凭证后才会继续轮转。
2.  **保护二（访问日志不可行）**：检查平台 AK 访问日志采集是否正常。若不可行，系统会在第一把队列即将禁用时停止轮转。
3.  **保护三（访问日志保护）**：检查平台 AK 访问日志，确认即将禁用的派生 AK 是否仍有产品在调用。若有调用记录，轮转将暂停。
4.  **检查 AUTO_ROTATE 状态**：检查 IAMID 中是否有 `CLOSE_AUTO_ROTATE` 状态，若有则表示该队列默认不轮转。
5.  **检查产品更新状态**：检查使用该产品的队列，是否有产品未及时更新（参考 平台凭证管理服务（PCM）介绍）。
6.  **检查 AK 使用代数**：检查使用该队列的产品中，是否有产品仍在使用第 7 把 AK（参考 平台凭证管理服务（PCM）介绍）。
...
*   应急处置预案：PCM应急处置
*   [[PCM/平台凭证管理服务/index|平台凭证管理服务]]介绍：平台凭证管理服务（PCM）介绍