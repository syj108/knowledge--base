...
graph TD
    ...
    P1 --> S1[核心排查场景]
    P2 --> S2[客户端WARN日志]
    P3 --> S3[Controller磁盘打满]
    P4 --> S4[Go SDK日志增长]
    P5 --> S5[Python RPM安装失败]
    P6 --> S6[Java线程阻塞]
    P7 --> S7[CLI工具报错]
```
...
这是 PCM 接入后最核心的排查场景，产品调用网关时可能报 AK 被禁用/无效/不存在。首先需判断是否是 PCM 禁用 AK 导致。
...
- **派生 AK 判定**：
  ...
  - 数据库查询：进入 `clm_db` 实例的 `pcm_db` 数据库，执行 `select * from ak_info where access_key_id='****';`。
...
1. 通常重启服务会刷新 AK 导致可用，然后停止该队列的轮转。
   ![重启服务](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/oJGq75k4LxgGzlAK/img/d4de6c7b-0ccf-4072-97a2-f3076bedea7c.png)
...
- **判断与处理**：这类 WARN 日志**不影响业务**（SDK 已降级返回原始凭证），主要影响是客户端告警监控被触发。2507 版本 PCM 服务端尚未部署或部分产品升级但 baseServiceAll 未升级时可能出现。
...
- **处理方式**：
  ...
  3. 清理历史日志文件（保留最近日志）。
  4. 排查原因：是否有大量异常请求持续打到 Controller，或定时任务异常导致循环报错。确认日志轮转配置是否正常。

#### Go SDK 日志文件持续增长
- **现象**：Go SDK 产生的日志文件不断增大，未按预期轮转。
- **原因**：Go SDK 在 2512 之前版本存在日志轮转 Bug。
- **解决方案**：升级 Go SDK 至 2512 及以上版本。临时处理：`> logfile` 截断日志文件（不要 rm 正在写入的文件）。
...
- **解决方案**：升级 SDK 至 `credprovider.plugin >= 1.0.8`。临时规避：JVM 参数 `-Djava.security.egd=file:/dev/./urandom`。
...
| `reason: "nbf" claim not valid until` | 时钟不同步 | 见下方 nbf 时钟偏差排查 |
| `token_arn not same with arn...` | ARN 不一致 | SDK 内部问题，基本不出现 |

**signature error 排查思路**：
签名 key = `sha256(initSK || IKM)`，其中 IKM = endpoint 去 `https://` -> 取域名 -> 去掉 `-regionid`。
...
1. **接入 PCM 后出现大量报错日志，是否有影响？**
   2507 版本 PCM 服务端尚未部署，或部分产品升级至 3186-2510 及以上版本但 baseServiceAll 未升级，可能访问 PCM 报错。因降级返回了原始底表 AK，不影响业务调用，但调用频繁可能产生大量错误日志。
2. **如何判断底表 AK 是否禁用？**
   在运维手册《PCM运维手册》中查询。
3. **如何判断派生 AK 禁用？**
   当前输出版本 3186、320 默认均不禁用派生 AK。
4. **时间敏感服务接入后延迟加大怎么办？**
   接入 PCM 后可导致部分时间敏感服务延迟加大。对于时间敏感服务，SDK（1.13-SNAPSHOT 20250908 及以上版本）增加了超时策略，支持 `PCM_TASK_DELAY` 环境变量，用于设置访问 PCM 最大超时时间（单位 ms，默认 1000ms 即 1s）。
...
- **半轮转模式首次获取失败导致后续持续异常**：部分产品采用半自动轮转模式（仅在启动时获取一次派生 AK，后续不再主动刷新）。如果该唯一一次获取请求恰好失败（如 Core 限流、网络抖动、服务未就绪），产品将持续使用底表 AK 或无有效凭据运行，且不会自动恢复。

### 业务与性能风险

- **链路增加延迟，对时间敏感业务有影响**：接入 PCM 后可能导致部分时间敏感服务延迟加大，且网络可能出现延迟。对于时间敏感服务，需合理配置 SDK 的 `PCM_TASK_DELAY` 环境变量以控制最大超时时间。

### SDK 行为与版本风险

- **无服务端时 SDK 频繁调用产生大量日志**：当环境中 PCM 服务（Core）未部署或不可达时，SDK 无法生成缓存，仍会按配置的间隔持续尝试连接，每次失败产生 WARN 级别日志。
- **部分 SDK 未打印关键日志，排查困难**：Java SDK WARN 日志过多，部分产品屏蔽了报错日志，导致无请求 PCM 的 RequestID 等关键信息，增加排查难度。
- **已知问题已修复但环境中存量版本旧**：
