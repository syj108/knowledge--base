...
当遇到访问报错，怀疑是 PCM 禁用 AK 导致时（如报 AK 被禁用/AK 无效/AK 不存在），优先通过拦截日志判定。提取日志中的请求 AK ID，并通过 PCM 服务查询 AK 状态及类型。如果已经禁用，采用对应的应急处置方案进行恢复，并反馈研发侧排查原因。
...
当发生组件故障或异常时，PCM 具备多级容错与降级保护能力，以保障业务连续性。以下为常见故障场景下的系统自动止血与降级行为：

| 故障场景 | 系统/SDK 自动降级行为 | 业务影响 |
| --- | --- | --- |
| 新部署时 PCM Core 还未 ready | SDK 将入参（原始底表 AK）作为凭证返回 | 无影响（Core 未禁用老 AK） |
| 运行时 PCM Core 宕机 | Core 末期过期老凭证行为暂停；SDK 返回上次获取的老凭证（未在窗口期末尾） | 无影响 |
| 产品独立升级，PCM 未 ready | SDK 将入参作为凭证返回 | 无影响 |
| PCM 和应用都挂了需重拉（SDK 缓存未丢失） | SDK 读取本地加密文件，返回上次获取的老凭证 | 无影响 |
| PCM 初始化服务异常或报错 | SDK 将入参作为凭证返回；若有缓存则返回最近一次从服务端获取的凭证 | 无影响 |
...
* **处理建议**：无需紧急止血，可直接忽略或调整告警规则，后续可排查网络或服务端状态。

## AK 调用网关被拦截应急恢复

这是 PCM 接入后最核心的排查场景。当产品调用网关时报 AK 被禁用/无效/不存在导致业务拦截时，首先需从网关日志中取出被拦截的 AK ID（可借助后文的**网关日志查询工具**），在控制台查询是底表 AK 还是派生 AK。

### 底表 AK (initAK) 被拦截

* **场景判断**：产品在使用底表 AK，说明 SDK 没有成功获取派生 AK，走了降级逻辑或使用底表 AK 未适配。
* **处置原则**：**先恢复**（在 PCM 控制台启用该底表 AK，立即恢复业务），**后排查**（查 SDK 日志 code，确认是哪种降级场景，排查为什么 SDK 没拿到派生 AK）。
...
当发生严重故障导致自动降级失效，或涉及高风险运维变更时，需采取人工应急操作进行止血与恢复。

### PCM 与应用双挂且 SDK 缓存丢失
* **故障表现**：PCM 和应用均宕机需重拉，且 SDK 本地缓存（内存与磁盘）丢失，导致无法获取凭证，**业务中断**。
* **应急止血操作**：
  * 优先使用**老凭证应急脚本**进行临时恢复，保障业务快速拉起。
  * 或紧急**恢复 PCM 服务**，待 PCM 服务就绪后重新获取派生凭证。

### 凭证管控模式变更异常（松→紧变更）
* **风险场景**：凭证管控模式从松到紧（如兼容模式向严格模式）变更时，可能引发预期外的凭证禁用，导致业务中断。
* **应急与防护操作**：
  * **系统机制保障**：模式从松到紧变更时**不自动生效**。
  * **人工介入**：必须通过 ASO 页面提示进行**人工处理**与确认，防止误操作导致业务凭证被意外禁用。

### 热升级后老凭证禁用异常
* **风险场景**：热升级后需禁用老凭证，直接一刀切禁用可能导致部分未改造完成或存在长连接的组件业务中断。
* **应急与恢复操作**：
  * **灰度禁用**：通过观测日志，在运维控制台（PKM 白屏管控）以运维变更方式逐步、灰度禁用老凭证。
  * **状态干预**：若发现禁用后业务异常，可通过 PCM Controller 的状态管理能力暂停禁用操作，排查日志关联 AK 使用记录，确认安全后再继续。
...
### Core 限流导致 HTTP 502
* **故障现象**：Core 返回 HTTP 502，大概率触发限流。PCM Core 的限流策略基于客户端 IP，同 IP 下高频产品可能耗尽配额导致其他产品被误伤。
* **应急操作**：
