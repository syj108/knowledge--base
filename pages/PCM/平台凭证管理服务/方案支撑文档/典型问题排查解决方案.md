...
### 一、问题描述
*   **问题现象**：[[PCM/PCM/index|PCM]] 接入后最核心的排查场景。产品调用各类网关（如 OSS、SLS、ASAPI、KMS、ODPS 等）时报 AK 被禁用 / AK 无效 / AK 不存在，请求被拦截（如返回 401、403 状态码）。
...
3.  **查询 AK 状态与终态判定**：通过 pcm 服务查询该 AK 的状态，确认其是否处于“已禁用”状态，并确认是底表 AK 还是派生 AK。
...
*   **日志示例**：
```json
{"__tag__:__hostname__": "c25g07018.cloud.g07.amtest17", "access_id": "5hN1RkUhRn43iNfw", "error_code": "InvalidAccessKeyId", "host": "cn-wulan-env17e-d01-as-console-cdn.oss-cn-wulan-env17e-d01-a.intra.env17e.shuguang.com", "ip": "10.17.46.36", "method": "GET", "operation": "GetBucketAcl", "request_id": "69EB1A0A3E6DA93539F3A4CE", "status": "403", "time": "24/Apr/2026:15:21:46", "url": "/?acl"}
```
...
*   **日志示例**：
```json
{"APIVersion": "0.6.0", "AccessKeyId": "cmchJQg057pBelHD", "ClientIP": "10.17.160.103", "Method": "GetConsumerGroupCheckPoint", "RequestId": "69EB0C444B76F491098A2F35", "Source": "10.17.160.103", "Status": "401", "UserAgent": "aliyun-log-sdk-java-0.6.64/1.8.0_412"}
```
...
*   **日志示例**：
```json
{"Method": "ListShards", "Status": "401", "ClientIP": "10.17.31.30", "AccessKeyId": "Khz7a1kmKMZDCBXj", "RequestId": "69D6169B34510383396636E7", "Source": "10.17.31.30", "ErrorCode": "Unauthorized", "ErrorMsg": "AccessKeyId is disabled: Khz7a1kmKMZDCBXj"}
```
...
*   **日志示例**：
```json
{"accessKeyId": "VidKjhddRaas4tMA", "apiName": "ListAllLevel1Orgs", "callerIp": "10.17.32.38", "errorCode": "asapi.server.request.parameter.accesskeyid.error", "errorMessage": "The specified AccessKey ID (VidKjhddRaas4tMA) is invalid. Details: (The Access Key is disabled.).", "errorSuggestion": "Check whether the AccessKey pair exists and is enabled.", "httpMethod": "POST", "regionId": "cn-wulan-env17e-d01", "requestId": "0a11243f17770122001463084d0062", "status": "failed"}
```
...
*   **日志示例**：
```json
{"api_name": "ListKeys", "error_code": "Forbidden.AccessKey", "error_message": "This AccessKey is not enabled.", "ip": "10.17.4.31", "region_id": "cn-wulan-env17e-d01", "request_id": "0efdb6f6-ae55-445e-b1e9-f514351d287b", "status_code": "403"}
```
...
*   **日志示例**：
```json
{"metadata": "{\"access_id\":\"fXWvhmRkMeER5QI6\",\"network_client_ip\":\"10.17.37.83\",\"vpc_id\":\"0\"}", "requests": "{\"url\":[\"/api/logview/host?curr_project=admin_task_project\",\"/api/projects?expectmarker=true&curr_project=admin_task_project\"]}"}
```
...
*   **实施步骤**：
    1.  采用 PCM应急处置 方案进行处置（如临时解禁或替换 AK）。
    2.  将问题现象、AK 信息及拦截日志反馈至研发侧，排查 AK 被禁用的根本原因。
...
#### 场景四：非本产品排查
*   **适用条件**：通过 pcm 服务查询 AK 状态为**正常（未禁用）**，但网关依然报错拦截；或平台AK访问日志/网络链路排查发现请求未到达PCM Core。
*   **实施步骤**：此情况**不属于** PCM 禁用 AK 导致的问题。需要联系对应报错网关（如 OSS、SLS、ASAPI、KMS、ODPS 等）的产品组或网关产品组，协助排查网关侧的鉴权逻辑或网络问题。
...
### 二、排查信息收集
*   **必须收集的信息**：IAMID、initAKID、AK申请记录详情。
*   **检查终态的方法**：登录PCM控制台（路径：ASO -> 安全管理 -> 账户安全 -> 平台凭据管理PCM），进入“AK申请详情”和“派生AK管理”标签页查看具体状态。
...
*   **实施步骤**：
    ...
    3.  检查使用该队列的产品中，是否有产品仍在第7把AK（参考《平台凭证管理服务（PCM）介绍》）。若因业务产品未更新导致轮转停止，需联系对应业务产品组协助升级更新。
...
*   **排查方向**：确认环境中 PCM 服务（Core）是否部署或可达。当无服务端时，SDK 无法生成缓存，仍然会按配置的间隔持续尝试连接，每次失败产生 WARN 级别日志。2507 版本 PCM 服务端尚未部署，或产品升级至 3186-2510 及以上版本但 `baseServiceAll` 未升级，均可能出现此问题。
*   **解决步骤**：确认 PCM 服务端状态，若未部署则忽略告警或部署服务端；若 `baseServiceAll` 未升级则进行升级。
...
*   **解决步骤**：确认 CLI 的 `pcm_endpoint` 指向正确的 PCM Core 地址。后续版本已优化解析异常的降级处理，升级至最新版本（2025-12-23及之后更新）可解决。
