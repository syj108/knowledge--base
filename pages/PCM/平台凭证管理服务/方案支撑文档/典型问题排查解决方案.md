...
### 一、问题描述
*   **问题现象**：[[PCM/PCM/index|PCM]] 接入后最核心的排查场景。用户或系统访问各云产品网关（如 OSS、SLS、ASAPI、KMS、ODPS 等）时出现访问报错、鉴权失败（如返回 401、403 状态码），怀疑是 PCM 禁用了 AccessKey（AK）导致的拦截。
...
*   **实施步骤**：此情况**不属于** PCM 禁用 AK 导致的问题。
    1.  **网关内部拦截**：如果 PCM 侧确认 AK 状态正常，且签名、时间戳均无误，但特定产品（如 OSS、SLS、ASAPI、KMS 等）依然拦截，需联系**对应网关产品组**协助排查其网关内部的鉴权缓存或特定拦截策略。
    2.  **权限策略拒绝**：如果报错信息明确为具体的业务权限不足（如 RAM Policy 拒绝、无操作权限），需联系 **RAM（资源访问管理）产品组**协助排查权限策略配置。
