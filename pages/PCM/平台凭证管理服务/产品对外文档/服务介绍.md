...
### 产品定位
平台凭证管理服务（PCM，Platform Credential Management）是 `baseServiceAll` 中的基础服务，核心目标是**接管平台底表 AK，实现凭证的动态轮换与安全管控**，提升安全性。
...
*   **接入端**：PCM SDK / CLI
*   **服务端**：PCM Core（缓存中间网关）、PCM Controller（策略中心）
*   **依赖服务**：UMM（AK 生命周期管理）、AAS（账户管理服务）
