...
*   **UMM / AAS**：作为 PCM 的核心依赖服务，负责底层 AK 生命周期管理和平台账户统一管理，与 PCM Controller 联动形成账户-凭证关联关系并执行轮换/禁用操作。
*   **标准网关（如 POP、OpenAPI、OSS 等）**：在标准 AK 认证场景下，标准网关通过对接 UMM 进行 AK 签名校验。当前访问标准 AK 认证服务的云产品均已适配完成。
*   **AK 私用服务**：服务不接或无法接 UMM，直接把 AK 参数记录到本地配置文件/数据库中，请求过来时用本地配置校验。当前访问 AK 私用服务的云产品尚未强制要求适配，已适配的产品通过 PCM 服务将兑换出原始底表 AK。

*(注：源文档未提及与 VPC、ECS、SLB 等具体 Top30 产品的交互方式，故不作编造。)*
