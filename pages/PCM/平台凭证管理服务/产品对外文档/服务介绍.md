...
| 异常场景 | SDK 行为 | 业务影响（边界） |
| --- | --- | --- |
| 新部署时 PCM Core 还未 ready | 将入参（底表AK）作为返回 | **不会造成业务中断**，业务使用底表 AK 正常运行，但会产生相关 WARN 日志。 |
| PCM 服务异常或网络超时 | 读取本地加密文件缓存或降级返回底表 AK | **不会造成业务中断**，优先使用本地缓存凭证，若无缓存则降级使用底表 AK，保障业务连续性。 |
| 底表 AK 被禁用且 PCM 服务不可用时重启服务 | 无法获取派生 AK，本地无缓存，底表 AK 已失效 | **会造成业务中断**。底表禁用后凭据供给完全依赖 PCM 链路，重启服务若遇 PCM 不可用将拿不到任何有效凭据。 |
| 接入 PCM 后的链路延迟 | 增加网络与服务调用耗时 | **可能影响时间敏感业务**。接入后可能导致部分时间敏感服务延迟加大，需通过 `PCM_TASK_DELAY` 环境变量调整超时时间（默认 1000ms）以缓解。 |
| 半轮转模式首次获取派生 AK 失败 | 降级使用底表 AK 或无凭据，后续不再主动刷新 | **可能造成后续持续异常**。半自动轮转模式仅在启动时获取一次，若首次失败则不会自动恢复。 |
| PCM 服务未部署或长期不可达 | 持续尝试连接并产生 WARN 日志 | **不会造成业务中断**（降级返回原始凭证），但会产生大量错误日志，可能触发客户端告警监控。 |