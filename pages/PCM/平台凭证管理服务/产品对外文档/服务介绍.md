...
- **超时控制**：支持通过 `PCM_TASK_DELAY` 环境变量设置访问 PCM 的最大超时时间（默认 1000ms），以优化时间敏感服务的延迟问题。
...
| PCM 服务（Core）未部署或不可达 | 持续尝试连接并产生大量 WARN 日志，降级返回原始凭证 | **不会造成业务影响**，但可能触发客户端告警监控 |
| 半轮转模式首次获取派生 AK 失败 | 首次获取失败后不再主动刷新，持续使用底表 AK 或无凭据 | **存在隐患**（若底表 AK 后续被禁用则业务中断） |
| 底表 AK 被禁用且 PCM 链路异常（无本地缓存） | 无法获取派生 AK，且底表 AK 已失效 | **会造成影响（业务直接中断）** |
| 接入 PCM 导致链路延迟增加 | 正常获取凭证，但增加网络与处理耗时 | **不会造成业务中断**，但对时间敏感业务有延迟影响（可通过 `PCM_TASK_DELAY` 控制超时） |