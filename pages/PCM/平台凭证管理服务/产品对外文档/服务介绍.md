...
- **白屏管控（PKM）与日志关联**：提供可视化的凭证管理界面，提供日志查询能力关联 AK 使用记录，判断是否可安全禁用。
...
### 与相关产品的交互方式及影响
- **UMM / AAS**：PCM Controller 依赖 UMM 进行 AK 的创建、获取、轮换和禁用，UMM 依赖 AAS 关联账户凭证。PCM 的指令直接影响 UMM 中 AK 的生命周期状态。
